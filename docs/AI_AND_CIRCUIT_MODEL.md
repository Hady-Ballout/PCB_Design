...
Circuit generation runs as a 3-stage pipeline (`runCircuitPipeline` in
`server/ai/ollamaProvider.ts`), each stage its own model call so a single free-tier rate
limit isn't shared across a whole request: a **circuit** stage returns `{"circuit": {...}}`,
a **reviewer** stage checks/corrects that circuit (skippable via `AI_PIPELINE_REVIEWER=0`),
and a **reply** stage writes the conversational `{"reply": "..."}` from the final circuit.
SPICE is never generated by the AI — it's always derived deterministically from the circuit
JSON via `toSpice()` (`src/core/pcbGenerator.js`), so no stage needs to author or match a
netlist by hand. The `circuit` object:
...
1. Ollama (or the OpenAI-compatible provider under `AI_PROVIDER`) is called per stage with
   its JSON schema as structured-output format (`CIRCUIT_SCHEMA`/`STAGE1_RESPONSE_SCHEMA`/
   `REVIEWER_RESPONSE_SCHEMA`/`REPLY_RESPONSE_SCHEMA`).
2. Each stage's parser repairs malformed/truncated JSON where possible.
3. `validateCircuitResponse` / `validateCircuit` (frontend) check schema conformance.
4. One automatic corrective retry (same model, `parseWithCorrectionRetry`) is triggered on
   invalid/malformed output per stage; reviewer/reply failures are non-fatal and fall back to
   the prior circuit / an empty reply rather than failing the whole request.
...
6. Streamed SPICE previews (derived from the circuit-stage's partial JSON via
   `buildStreamingSpice`) are marked `provisional`/`correcting` until the full circuit
   validates.
